...
class Pagy
  ...
  module Frontend
    ...
    # It's ~12x faster and uses ~6x less memory than I18n.t
    # See https://ddnexus.github.io/pagy/extras/i18n if you need to use the standard I18n gem instead
    def pagy_t(path, vars={})
      value_proc = I18N[:data][path] || vars[:count] && I18N[:data][path+=".#{I18N[:plural].call(vars[:count])}"] or return %(translation missing: "#{path}")
